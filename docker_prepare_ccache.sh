...
if [[ -f "/etc/centos-release" ]]; then
    ...
    for i in $(seq 1 5); do 
        yum install -y zlib-devel curl-devel expat-devel libpng-devel ccache wget ninja-build && s=0 && break || s=$? && sleep 15;
    done
...
HOST_CCACHE_DIR="/host${HOST_CCACHE_DIR:-/home/runner/work/cibuildwheel-llvm-mlir/cibuildwheel-llvm-mlir/.ccache}"
