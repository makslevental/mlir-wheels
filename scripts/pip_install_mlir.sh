...
export PIP_FIND_LINKS="https://github.com/makslevental/mlir-wheels/releases/expanded_assets/latest"

SITE_PACKAGES=$(python -c "import site; print(site.getsitepackages()[0])")

if [ ! -z "$MLIR_COMMIT" ]; then
  pip install mlir-native-tools==$MLIR_COMMIT --force -U

  DATE=$(echo $MLIR_COMMIT | cut -d "+" -f 1)
  HASH=$(echo $MLIR_COMMIT | cut -d "+" -f 2)
  LOCAL_VERSION=""
  ...
  MLIR_COMMIT="==$DATE+$LOCAL_VERSION"
else
